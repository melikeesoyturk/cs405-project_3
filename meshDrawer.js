...
void main()
{
	...
	float dotNL = dot(normal, lightDirNormalized);
	diff = max(dotNL, 0.0);
	
	// Specular lighting calculation
	vec3 cameraPosition = vec3(0, 0, -1.0);
	vec3 toCameraVec = normalize(cameraPosition - vPosition);
	vec3 reflectionVec = normalize(reflect(lightDirNormalized, normal));
	float cosTheta = dot(reflectionVec, toCameraVec);
